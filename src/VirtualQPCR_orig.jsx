...
const TICK = { fill: "#bbc", fontSize: 13 };
const AXIS_LABEL_STYLE = { fill: "#bbc", fontSize: 13 };
const CHART_MARGIN = { top: 10, right: 24, bottom: 28, left: 10 };
const TT_STYLE = { background: "#1a2233", border: "1px solid #445", borderRadius: 8, fontSize: 13 };
    ...
    <span style={{ position: "relative", display: "inline-block" }}>
      <button
        ...
        style={{
          ...
          lineHeight: "26px", textAlign: "center", fontFamily: "serif",
        }}
      >
        i
      </button>
      {open && (
        <div style={{
          ...
          background: "#1a2233", color: "#e0e8f0",
          padding: "16px 18px", borderRadius: 12, fontSize: 15,
          lineHeight: 1.6, zIndex: 999,
          boxShadow: "0 8px 30px rgba(0,0,0,0.4)",
          border: "1px solid rgba(255,255,255,0.12)",
        }}>
          <div style={{ fontWeight: 700, marginBottom: 8, color: "#3388cc", fontSize: 16 }}>
            {title}
          </div>
          {children}
          <button
            ...
            style={{
              ...
              color: "#99a", cursor: "pointer", fontSize: 20,
            }}
          >
            ×
          </button>
...
function Btn({ children, onClick, variant, disabled }) {
  ...
  return (
    <button
      ...
      style={{
        ...s, padding: "12px 24px", borderRadius: 10, fontSize: 16,
...
function Card({ title, info, children }) {
  return (
    <div style={{
      ...
      {title && (
        <h3 style={{ margin: "0 0 14px", fontSize: 20, fontWeight: 600, color: "#dde" }}>
          {title}
    ...
    <div style={{ display: "flex", gap: 6, marginBottom: 22 }}>
      {stg.map((s, i) => (
        <div key={i} style={{ flex: 1, textAlign: "center" }}>
          ...
          <div style={{
            fontSize: 12, color: i <= idx ? "#3388cc" : "#556",
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 12, color: "#889", textTransform: "uppercase", letterSpacing: 1 }}>Time</div>
        <div style={{ fontSize: 26, fontWeight: 700, color: days > 14 ? "#e85" : "#8cf" }}>
          {days.toFixed(1)} <span style={{ fontSize: 14, fontWeight: 400 }}>days</span>
        </div>
      </div>
      <div style={{ width: 1, background: "rgba(255,255,255,0.1)" }} />
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 12, color: "#889", textTransform: "uppercase", letterSpacing: 1 }}>Cost</div>
        <div style={{ fontSize: 26, fontWeight: 700, color: cost > 1000 ? "#e85" : "#8cf" }}>${cost}</div>
      </div>
    </div>
  );
}
...
function GelSVG({ lanes, label }) {
  const lW = 48, gap = 66, w = 24 + lanes.length * gap, h = 360;
  return (
    <div style={{ textAlign: "center" }}>
        ...
        {lanes.map((lane, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={cx} y={h - 8} fill="#99a" fontSize={12} textAnchor="middle" fontFamily="sans-serif">
                {lane.label}
              </text>
              {lane.smear && (
      ...
      {label && <div style={{ fontSize: 14, color: "#99a", marginTop: 8 }}>{label}</div>}
    </div>
  );
}
...
function NanoDropDisplay({ concentration, r280, r230 }) {
  return (
    <div style={{
      ...
      padding: "22px 28px", maxWidth: 440, margin: "0 auto",
      fontFamily: "'Courier New', monospace",
      boxShadow: "0 6px 24px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.4)",
      border: "2px solid #7a8a9a",
    }}>
      <div style={{ background: "#1a3050", color: "#5599cc", padding: "4px 12px", borderRadius: 5, fontSize: 13, marginBottom: 12 }}>
        BFW / Nucleic acid: RNA
      </div>
      <div style={{ fontSize: 12, color: "#556" }}>Concentration nucleic acid</div>
      <div style={{ fontSize: 38, fontWeight: 700, color: "#111", textAlign: "center", margin: "8px 0 12px" }}>
        {concentration.toFixed(1)} <span style={{ fontSize: 18 }}>ng/µL</span>
      </div>
      <div style={{ display: "flex", justifyContent: "space-around", borderTop: "1px solid #889", paddingTop: 12 }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 12, color: "#556" }}>A260/A280</div>
          <div style={{ fontSize: 24, fontWeight: 600, color: r280 >= 1.8 && r280 <= 2.15 ? "#1a7a3a" : "#b33" }}>
            {r280.toFixed(3)}
          </div>
        </div>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 12, color: "#556" }}>A260/A230</div>
          <div style={{ fontSize: 24, fontWeight: 600, color: r230 >= 1.8 ? "#1a7a3a" : "#b33" }}>
            {r230.toFixed(3)}
          </div>
        </div>
      </div>
...
export default function VirtualQPCR() {
  ...
  // Body text style
  const P = ({ children }) => <p style={{ color: "#b0b8c4", lineHeight: 1.8, fontSize: 16, marginBottom: 18 }}>{children}</p>;
    ...
    switch (step) {
      case "intro":
        return (
          <div style={{ textAlign: "center" }}>
            ...
            <h1 style={{ fontSize: 32, fontWeight: 300, marginBottom: 12, letterSpacing: -0.5 }}>Virtual qPCR Flow</h1>
            <p style={{ color: "#99aabb", lineHeight: 1.8, maxWidth: 540, margin: "0 auto 28px", fontSize: 17 }}>
            ...
            <div style={{ maxWidth: 300, margin: "0 auto 24px" }}>
              <input value={playerName} onChange={e => setPlayerName(e.target.value)} placeholder="Enter your name"
                style={{ width: "100%", padding: "12px 16px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.05)", color: "#dde", fontSize: 17, fontFamily: "inherit", outline: "none", textAlign: "center" }} />
            </div>
      ...
      case "primer_search": {
        ...
        return (
          ...
          >
            <div style={{ fontSize: 14, color: "#778", marginBottom: 14 }}>
              Viewing primer set {primerIdx + 1} of {PRIMERS.length}
            </div>
            <P>{p.desc}</P>
            <PrimerTable primer={p} />
            <div style={{ marginTop: 14, fontSize: 15, color: "#b0b8c4" }}>
              Amplicon: <strong>{p.ampBp} bp</strong> · Predicted Tm: <strong>~{p.tm}°C</strong>
            </div>
            {p.ampBp > 300 && (
              <div style={{ background: "rgba(255,150,50,0.1)", border: "1px solid rgba(255,150,50,0.3)", borderRadius: 10, padding: "12px 16px", margin: "12px 0", fontSize: 15, color: "#eb8" }}>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 18, flexWrap: "wrap" }}>
              ...
              {isLast && (
                <div style={{ width: "100%", fontSize: 15, color: "#b0a080", marginTop: 4 }}>
        ...
        return (
          <Card title={`${viewPrimer.label} — Amplicon`} info={<span>The <strong>amplicon</strong> is the DNA produced by PCR. For qPCR, <strong>80–200 bp</strong> amplicons are ideal — they amplify efficiently and give sharp melt peaks. Amplicons &gt;300 bp amplify poorly and may fail while smaller housekeeping gene amplicons succeed.</span>}>
            ...
            {viewPrimer.ampBp > 300 && (
              <div style={{ background: "rgba(255,150,50,0.1)", border: "1px solid rgba(255,150,50,0.3)", borderRadius: 10, padding: "12px 16px", margin: "0 0 12px", fontSize: 15, color: "#eb8" }}>
                ⚠ Very large amplicon for qPCR — amplification efficiency will be reduced.
              </div>
            )}
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={makeSingleMelt([{ a: 18, mu: viewPrimer.tm, s: 1.5 }])} margin={CHART_MARGIN}>
                ...
                <Line type="monotone" dataKey="dFdT" name="Predicted" stroke="#ff6644" strokeWidth={2.5} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          ...
          <Card title="UV Absorption Spectrum" info={<span>Good RNA: <strong>single peak at 260 nm</strong>. A shoulder at <strong>280 nm</strong> indicates protein. High absorbance at <strong>230 nm</strong> indicates organic contaminants.</span>}>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={spectrumData} margin={CHART_MARGIN}>
                ...
                <Tooltip contentStyle={TT_STYLE} />
                <Line type="monotone" dataKey="abs" stroke="#4488ff" strokeWidth={2.5} dot={false} />
          ...
          <Card title="Amplification Plot" info={<span>Shows <strong>ΔRn vs Cycle</strong>. The <strong>Ct</strong> (threshold cycle) is where the curve crosses the threshold — lower Ct means more starting template. Blue = <strong>HK</strong> (housekeeping gene). Red = <strong>GOI</strong> (gene of interest). Dashed = 1:8 dilutions (should shift ~3 cycles right). Use <strong>log scale</strong> to see the exponential phase.</span>}>
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10, gap: 8 }}>
              {["Linear", "Log"].map(l => (
                <button key={l} onClick={() => setLogScale(l === "Log")}
                  style={{ padding: "6px 18px", borderRadius: 6, border: "2px solid #3388cc", fontSize: 14, cursor: "pointer", fontWeight: 600, background: (l === "Log") === logScale ? "#3388cc" : "transparent", color: (l === "Log") === logScale ? "#fff" : "#3388cc" }}>{l}</button>
              ))}
            </div>
            <ResponsiveContainer width="100%" height={360}>
              <LineChart data={ampData} margin={CHART_MARGIN}>
                ...
                <Line type="monotone" dataKey="hk" name="HK (undiluted)" stroke="#4488ff" strokeWidth={2.5} dot={false} />
                <Line type="monotone" dataKey="goi" name="GOI (undiluted)" stroke="#ff4444" strokeWidth={2.5} dot={false} />
                <Line type="monotone" dataKey="hkDil" name="HK (1:8 dilution)" stroke="rgba(68,136,255,0.5)" strokeWidth={2} dot={false} strokeDasharray="6 4" />
                <Line type="monotone" dataKey="goiDil" name="GOI (1:8 dilution)" stroke="rgba(255,68,68,0.5)" strokeWidth={2} dot={false} strokeDasharray="6 4" />
          ...
          <Card title="Melt Curve Analysis" info={<span>Plots <strong>−dF/dT vs temperature</strong>. Each peak represents a distinct PCR product melting. A <strong>single sharp peak</strong> per target means specific amplification. <strong>Multiple peaks</strong> indicate non-specific products. A <strong>low-temperature peak (65–75°C)</strong> usually indicates primer dimers. Blue = HK, Red = GOI.</span>}>
            <ResponsiveContainer width="100%" height={360}>
              <LineChart data={meltData} margin={CHART_MARGIN}>
                ...
                <Tooltip contentStyle={TT_STYLE} />
                <Legend wrapperStyle={{ fontSize: 14, paddingTop: 8 }} />
                <Line type="monotone" dataKey="hk" name="Housekeeping gene" stroke="#4488ff" strokeWidth={2.5} dot={false} />
                <Line type="monotone" dataKey="goi" name="Gene of interest" stroke="#ff4444" strokeWidth={2.5} dot={false} />
              </LineChart>
            </ResponsiveContainer>
            <div style={{ display: "flex", gap: 12, marginTop: 18, justifyContent: "center" }}>
      ...
      case "end":
        return (
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: 64, marginBottom: 14 }}>{isSuccess ? "🎉" : "😞"}</div>
            <h2 style={{ fontSize: 28, fontWeight: 400, marginBottom: 14, color: isSuccess ? "#8c8" : "#e88" }}>
              {isSuccess ? "Experiment Successful!" : "Experiment Not Optimised"}
            </h2>
            <p style={{ color: "#b0b8c4", lineHeight: 1.8, maxWidth: 520, margin: "0 auto 24px", fontSize: 17 }}>
              {isSuccess
            ...
            <div style={{ display: "inline-block", padding: "20px 32px", borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", marginBottom: 24 }}>
              <div style={{ display: "flex", gap: 40 }}>
                <div>
                  <div style={{ fontSize: 13, color: "#889" }}>TIME</div>
                  <div style={{ fontSize: 32, fontWeight: 300 }}>{days.toFixed(1)} <span style={{ fontSize: 15 }}>days</span></div>
                </div>
                <div>
                  <div style={{ fontSize: 13, color: "#889" }}>COST</div>
                  <div style={{ fontSize: 32, fontWeight: 300 }}>${cost}</div>
                </div>
                <div>
                  <div style={{ fontSize: 13, color: "#889" }}>RESULT</div>
                  <div style={{ fontSize: 32, fontWeight: 300, color: isSuccess ? "#8c8" : "#e88" }}>{isSuccess ? "✓" : "✗"}</div>
                </div>
              </div>
            </div>
